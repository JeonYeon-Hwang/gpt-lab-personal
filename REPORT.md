...
다음은 버그 수정 목록입니다:  
1. train_epoch_sentiment에서 `model_train`누락:  
다음 학습 때 `train()`을 호출하지 않아, drop_out이 꺼진 상태로 진행  

2. 가중치를 중복해서 더함 `out_E = delta_E + x`:  
이미 x = self.att(x) 에서 delta_E + x를 시행 중, 중복해서 더하는 문제 발생  

3. 마지막 단어가 pad로 오인됨 `hidden_states[:, -1, :]`:  
...
**수정 방향**  
수학적 기반 이론: 단어의 의미는 관계에서 창발한다. 즉, 유의미한 단어 간 관계가 多해야. 

현재 인자값 상태:
단어장 2000개(이론적으로 조합 200만) ↔ 학습 데이터(2만 단어: 유효한 조합 10-20만)  
조합에는 유효한 조합(긍정/부정 변별 o)과 중복 조합이 있다. 상당수는 중복 조합.  
<br>
